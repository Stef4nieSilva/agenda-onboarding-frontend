...
export default function GradeHorariosDia({ agendamentos, agentes, setAlunoSelecionado, horarios, ausencias }) {
...
const { entrada, inicioIntervalo, fimIntervalo, fimExpediente } = ausencia;

const estaAusente =
  !entrada || !fimExpediente ||  // Se não definiu entrada ou fim, considera ausente
  (
    horario < entrada || 
    (inicioIntervalo && fimIntervalo && horario >= inicioIntervalo && horario < fimIntervalo) ||
    (fimExpediente && horario >= fimExpediente)
          ...
          return (
            <div key={agente + horario} className="bloco-agente">
              {estaAusente ? (
                <div className="bloco-agente-conteudo ausente">
                  <span className="texto-ausente">Ausente</span>
                </div>
              ) : agendamento ? (
                <div
                  className={`bloco-agente-conteudo ${
                    agendamento.status === "resolvido" ? "resolvido" : ""
                  }`}
